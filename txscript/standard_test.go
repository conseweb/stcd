...
// newAddressPubKey returns a new coinutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) coinutil.Address {
	addr, err := coinutil.NewAddressPubKey(serializedPubKey,
...
// newAddressScriptHash returns a new coinutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) coinutil.Address {
	addr, err := coinutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the coinutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the coinutil.Address interface.
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := coinutil.NewAddressPubKey(decodeHex("02192d7"+
	...
	p2pkUncompressedMain, err := coinutil.NewAddressPubKey(decodeHex("0411d"+
	...
	tests := []struct {
		keys      []*coinutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*coinutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
